...
namespace NET03_03_7_BinarySearchTree
{
    ...
    public class BinarySearchTree<T>  : IEnumerable<T>
    {
        ...
        public IEnumerable<T> Values
        {
            get
            {
                return this.Select(nodeData => nodeData);
            }
        }
        ...
        public IEnumerable<T> Traverse(BinaryTreeNode<T> startingNode)
        {
            switch (this.TraversalOrder)
            {
                case TraversalOrder.PreOrder:
                    if (startingNode is null)
                        yield break;
                    yield return startingNode.Data;
                    if (! (startingNode.Left is null))
                    {
                        foreach (T nodeData in Traverse(startingNode.Left))
                        {
                    ...
                    {
                        foreach (T nodeData in Traverse(startingNode.Right))
                        {
                            yield return nodeData;
                        }
                    }
        ...
        {
            return Traverse(this.Root).GetEnumerator();
        }
